...
gcoinDB.on('connected', function(){
  console.log('Gcoin-bank - Mongoose connected to ' + dbURI);
  require('./identities');
});
gcoinDB.on('error', function(err){
  console.log('Gcoin-bank - Mongoose connection error: ' + err);
});
gcoinDB.on('disconnected', function(){
  console.log('Gcoin-bank - Mongoose disconnected');
});

const gracefulShutdown = function(msg, callback){
  gcoinDB.close(function(){
    console.log('Gcoin-bank - Mongoose disconnected through ' + msg);
    callback();
  });
};
